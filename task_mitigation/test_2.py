...
SUB_PATH = 'bert_base_submission_qux.csv'
#BERT_PATH = 'model/uncased_L-4_H-256_A-4'
BERT_PATH = 'bert-base-uncase'
output_model_file = 'output/bert_base_1.bin'
...
for step, batch in enumerate(tqdm(test_loader, desc="validation", unit="batch")):
  ...
  # 不让模型计算或存储梯度，节省内存和加速预测
    ...
    # aux = sig[:, 1:]
    aux = sig[:,3:4]
    # max_aux, _ = torch.max(aux, dim=1)
    result = target.squeeze()-0.0514*aux.squeeze()
